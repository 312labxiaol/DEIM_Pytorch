...
class LearnableAffineBlock(nn.Module):
    ...
    def __init__(
            self,
            scale_value=1.0,    # 初始化缩放因子，默认为1.0
            bias_value=0.0  #初始化偏移量，默认为0.0（无偏移）
    ):
        super().__init__()
        # 定义可学习参数：缩放因子和偏移量
...
class ConvBNAct(nn.Module):
    """
    卷积 + 批归一化 + 激活函数模块
    该模块组合了卷积层、批归一化层和激活函数，用于特征提取和处理。
    """
    def __init__(
            self,
            in_chs,         # 输入通道数
            ...
            padding='',     # 填充方式，默认为空（自动计算）
            use_act=True,   # 是否使用激活函数，默认为True
            use_lab=False   # 是否使用Lab 颜色空间的特殊处理（默认为False）
    ):
        super().__init__()
        self.use_act = use_act
        self.use_lab = use_lab
        if padding == 'same':
            # 使用'same'填充时，手动添加ZeroPad2d以保持输入输出尺寸一致
...
class StemBlock(nn.Module):
    """
    HGNetv2的Stem Block，作为网络的输入处理模块
    ...
    Args：
        ...
        mid_chs (int): 中间层通道数。
        out_chs (int): 输出通道数。
        use_lab (bool): 是否使用Lab 颜色空间的特殊处理（默认为False）
    """
    # for HGNetv2
